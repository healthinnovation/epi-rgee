...
temp <- Map$addLayer(past,visParams = viz) + 
  Map$addLegend(visParams = viz,name = "Tem[C°]",position = "bottomleft")
temp
```
...
dataset <- ee$ImageCollection$Dataset$MODIS_006_MYD13Q1
current <- dataset$
  select("NDVI")$
...
ndvi <- Map$addLayer(current,visParams = viz) + 
